...
class EmailService {
  ...
  private generateOrderEmailHTML(data: OrderEmailData, isAdmin: boolean = false): string {
  ...
  async sendNewOrderNotificationToAdmin(data: AdminOrderEmailData): Promise<void> {
    if (!this.transporter) {
  ...
  async sendOrderConfirmationToCustomer(data: CustomerOrderEmailData): Promise<void> {
    if (!this.transporter) {
