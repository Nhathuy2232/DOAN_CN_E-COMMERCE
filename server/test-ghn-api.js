 ...
 * 
 * Cách sử dụng:
 * 1. Cấu hình .env với GHN credentials trong thư mục server/
 * 2. Chạy từ thư mục server/: node test-ghn-api.js
...
// ==================== GHN CONFIGURATION ====================
const GHN_CONFIG = {
  baseUrl: process.env.GHN_BASE_URL || 'https://online-gateway.ghn.vn',
  token: process.env.GHN_TOKEN,
  shopId: process.env.GHN_SHOP_ID,
};

// Debug: Kiểm tra token có được load không
console.log('\n🔧 GHN Configuration Debug:');
console.log(`   Base URL: ${GHN_CONFIG.baseUrl}`);
console.log(`   Token từ env: ${process.env.GHN_TOKEN ? 'CÓ' : 'KHÔNG'}`);
console.log(`   Token (10 ký tự đầu): ${GHN_CONFIG.token ? GHN_CONFIG.token.substring(0, 10) + '...' : 'KHÔNG CÓ'}`);
console.log(`   Shop ID: ${GHN_CONFIG.shopId}`);
console.log(`   File .env có tồn tại: ${require('fs').existsSync('.env') ? 'CÓ' : 'KHÔNG'}\n`);

// Axios client cho GHN API (base config)
// Lưu ý: Một số API yêu cầu 'token' (chữ thường), một số yêu cầu 'Token' (chữ hoa)
...
// Helper function để tạo headers cho từng loại API
function getHeaders(apiType = 'default') {
  const headers = {
    'Content-Type': 'application/json'
  };
  ...
  switch(apiType) {
    case 'master-data':
      // Master Data API: chỉ cần token (chữ thường hoặc chữ hoa đều được)
      headers['Token'] = GHN_CONFIG.token;
      break;
    case 'get-service':
    ...
    case 'return':
      ...
      headers['token'] = GHN_CONFIG.token;
      headers['shop_id'] = GHN_CONFIG.shopId;
      break;
    default:
      // Mặc định: Token (chữ hoa)
      headers['Token'] = GHN_CONFIG.token;
      headers['ShopId'] = GHN_CONFIG.shopId;
  }
...
async function getProvinces() {
  try {
    ...
    const response = await ghnClient.get('/shiip/public-api/master-data/province', {
      headers: getHeaders('master-data')
    });
    ...
    // Hướng dẫn nếu token không hợp lệ
    if (error.response?.data?.code === 401 || error.response?.data?.message?.includes('Token')) {
      console.error('\n⚠️  TOKEN KHÔNG HỢP LỆ!');
      console.error('📝 Cách khắc phục:');
      console.error('   1. Đăng nhập vào: https://sso.ghn.vn/manage-ip');
      console.error('   2. Copy Token API mới từ trang quản lý');
      console.error('   3. Cập nhật GHN_TOKEN trong file server/.env');
      console.error('   4. Hoặc cập nhật trực tiếp trong file test-ghn-api.js (dòng 21)');
      console.error(`\n   Token hiện tại: ${GHN_CONFIG.token.substring(0, 20)}...`);
    }
...
async function getAvailableServices(fromDistrict, toDistrict) {
  try {
    ...
    const response = await ghnClient.post(
      ...
      requestData,
      {
        headers: getHeaders('get-service')
      }
    );
...
async function createShippingOrder(orderData) {
  try {
    ...
    const requestData = {
      ...
      // Thông tin người gửi
      from_name: orderData.fromName || 'nhathuy',
      from_phone: orderData.fromPhone || '0376911677',
...
async function showMenu() {
  ...
  console.log('\n📌 GHN Configuration:');
  console.log(`   Base URL: ${GHN_CONFIG.baseUrl}`);
  console.log(`   Token: ${GHN_CONFIG.token ? GHN_CONFIG.token.substring(0, 10) + '...' : 'NOT SET'}`);
  console.log(`   Shop ID: ${GHN_CONFIG.shopId}`);
  console.log('\n📋 Available Tests:');
...
async function main() {
  // Kiểm tra cấu hình
  if (!GHN_CONFIG.token || GHN_CONFIG.token === 'your-ghn-token-here' || !GHN_CONFIG.shopId || GHN_CONFIG.shopId === 'your-shop-id-here') {
    console.log('\n⚠️  CẢNH BÁO: Chưa cấu hình GHN credentials!');
    console.log('📝 Vui lòng cập nhật file .env trong thư mục server/ với:');
    console.log('   GHN_BASE_URL=https://dev-online-gateway.ghn.vn');
